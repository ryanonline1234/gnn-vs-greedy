...
(Apple M1 Pro, 10 core, 32 GB). 559 measurement records in `results/*.jsonl`
(12 files). The measurements are the author's, produced by running the code on his
...
| — | undisclosed operating ceiling in d | **Fails for d ≥ 8** (raw output non-empty in 0/87 runs pooled over d ≥ 8, n=1000, in the released implementation's configuration d0 = int(√n); also 0/15 at the paper's d0 = int(∛n) = 10 at d = 8, 12, 20, D39) | `results/collapse.jsonl`, `escape.jsonl`, `phase1.jsonl` — §4 |
...
escape in 10/10 runs and d = 8, 12, 20 in 0/15 (Wilson 95% CI
[0.000, 0.204]) — the ceiling holds at that width too, at the degrees tested.
...
3. **The ceiling at the paper's width rests on 15 runs.** D39 re-ran d = 3, 5, 8, 12, 20 at the
   paper's d0 = int(∛n) = 10: 10/10 escape at d = 3 and 5, 0/15 at d ≥ 8.
